...
net::task<bool> socket::handshake() noexcept {
  while (tls_) {
...
net::task<std::string_view> socket::tls_recv(char* data, std::size_t size) noexcept {
  while (true) {
    ...
    default:
#ifdef _DEBUG
      std::cerr << "socket::tls_recv: " << ::tls_error(tls_.get()) << '\n';
#endif
      co_return std::string_view{};
    }
  }
...
net::task<bool> socket::tls_send(std::string_view data) noexcept {
  ...
  while (buffer_size) {
    ...
    default:
#ifdef _DEBUG
      std::cerr << "socket::tls_send: " << ::tls_error(tls_.get()) << '\n';
#endif
      co_return false;
    }
  }
...
net::task<std::string_view> socket::native_recv(char* data, std::size_t size) noexcept {
  ...
  if (WSARecv(handle_, &buffer, 1, &bytes, &flags, &event, nullptr) == SOCKET_ERROR) {
...
#ifdef _DEBUG
      std::cerr << "socket::native_recv: " << std::error_code(code, std::system_category()).message() << '\n';
#endif
      co_return std::string_view{};
    }
    ...
    if (!bytes) {
      ...
      if (const auto code = WSAGetLastError()) {
        std::cerr << "socket::native_recv (async): " << std::error_code(code, std::system_category()).message() << '\n';
      }
#endif
      co_return std::string_view{};
    }
...
net::task<bool> socket::native_send(std::string_view data) noexcept {
  ...
  while (buffer.len > 0) {
    ...
    if (WSASend(handle_, &buffer, 1, &bytes, 0, &event, nullptr) == SOCKET_ERROR) {
      if (const auto code = WSAGetLastError(); code != ERROR_IO_PENDING) {
#ifdef _DEBUG
        std::cerr << "socket::native_send: " << std::error_code(code, std::system_category()).message() << '\n';
#endif
        co_return false;
      }
      bytes = co_await event;
      if (!bytes) {
        ...
        WSAGetOverlappedResult(handle_, &event, &bytes, FALSE, &flags);
        if (const auto code = WSAGetLastError()) {
          std::cerr << "socket::native_send (async): " << std::error_code(code, std::system_category()).message() << '\n';
        }
#endif
        co_return false;
      }
